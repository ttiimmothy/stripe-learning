      ...
      {products.map((product, index) => (
        <div key={product._id} className="flex flex-col md:flex-row md:items-center md:justify-between shadow-md md:p-5 p-2 mb-4">
          <div className="flex items-center">
            <span className="size-6 mr-4 bg-primary text-white rounded-full text-sm flex items-center justify-center">{String(index + 1).padStart(2, '0')}</span>
            <Image src={product.image} alt={product.name} width={100} height={100} className="size-12 object-cover mr-4"/>
          <div>
            <h5 className="text-lg font-medium min-w-40 w-full whitespace-nowrap text-ellipsis overflow-hidden">{product.name}</h5>
          ...
          <div className="ml-5"><button className="text-red-500 hover:text-red-800 mr-4" onClick={(e) => handleRemove(e, product._id)}>Remove</button></div>
          </div>
        </div>
      ))}
    </div>
